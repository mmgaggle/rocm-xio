...
/**
 * @brief NVMe Key-Value operation selected by @c --kv-op (host-side config).
 *
 * Strongly-typed replacement for the former free-form @c std::string: parsing
 * is validated at the CLI layer (CheckedTransformer), and the value is lowered
 * to the numeric wire opcode (@c nvme_kv_cmd_store etc.) in nvmeIoParams.
 */
enum class KvOp : uint8_t {
  None = 0, /**< No KV op: normal block I/O mode. */
  Store,    /**< KV Store. */
  Retrieve, /**< KV Retrieve. */
  Exec,     /**< KV Exec (vendor 0x83). */
};
...
struct nvmeEpConfig {
  ...
  struct {
    ...
    bool infiniteMode;         /**< Run until stopRequested is set. */
    uint32_t batchSize;        /**< SQEs per doorbell; 1=serial, 0=all. */
    /* KV mode (kvOp == KvOp::None => normal block mode). Trailing defaulted
     * members so the brace-init in the constructor stays valid. */
    KvOp kvOp = KvOp::None; /**< KV op: Store/Retrieve/Exec, or None (block). */
